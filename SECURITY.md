...
Tooling that harvests primary legal sources into the identity-law corpora. In scope: anything in the fetching or manifest machinery that would let a harvested document be silently substituted for another, since the whole point is to quote sources without re-verifying them by hand.
